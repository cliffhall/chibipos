...
const { Sequelize: ResolvedSequelizeConstructor, Op: ResolvedOp } =
  sequelizePackage;

if (typeof ResolvedSequelizeConstructor !== "function") {
  const errorMsg =
    "[Main Index] Critical: ResolvedSequelizeConstructor is not a constructor function.";
  console.error(
    errorMsg,
    "Type:",
    typeof ResolvedSequelizeConstructor,
    "Package keys:",
    Object.keys(sequelizePackage).join(", "),
  );
  if (dialog && typeof dialog.showErrorBox === "function") {
    dialog.showErrorBox(
      "Initialization Error",
      "Failed to load database library (Sequelize). The application cannot start.",
    );
  }
  if (
    app &&
    typeof app.quit === "function" &&
    (typeof app.isQuitting !== "function" || !app.isQuitting())
  ) {
    app.quit();
  }
...
console.log(
  "[Main Index DEBUG] Raw VITE_DEV_SERVER_URL from process.env:",
  VITE_DEV_SERVER_URL,
);

const isDev = !!VITE_DEV_SERVER_URL;
console.log(
  `[Main Index DEBUG] Parsed VITE_DEV_SERVER_URL: ${VITE_DEV_SERVER_URL}, isDev: ${isDev}`,
);

const CRYPTO_KEY =
  process.env.CHIBIPOS_CRYPTO_KEY || "your-default-super-secret-key-for-dev";
if (CRYPTO_KEY === "your-default-super-secret-key-for-dev" && !isDev) {
  console.warn(
    "[Main Index] WARNING: Using default CRYPTO_KEY in production. This is insecure!",
  );
}
...
async function initializeDatabase() {
  try {
    const { sequelize, testConnection, dbPath } = initializeSequelize(
      app,
      ResolvedSequelizeConstructor,
    );
    ...
    if (!isDev) {
      ...
      const resourcesDbPath = path.join(
      ...
      if (!fs.existsSync(userDataDbDir)) {
        ...
        console.log(
          `[Main Index DB] Created userData directory: ${userDataDbDir}`,
        );
      }

      if (!fs.existsSync(userDataDbPath) && fs.existsSync(resourcesDbPath)) {
        console.log(
          `[Main Index DB] Database not found at ${userDataDbPath}. Copying from ${resourcesDbPath}...`,
        );
        fs.copyFileSync(resourcesDbPath, userDataDbPath);
        console.log(
          `[Main Index DB] Database copied successfully to ${userDataDbPath}.`,
        );
      } else if (
        !fs.existsSync(resourcesDbPath) &&
        !fs.existsSync(userDataDbPath)
      ) {
        console.error(
          `[Main Index DB] Packaged database not found at ${resourcesDbPath} and no existing DB at ${userDataDbPath}. Cannot proceed.`,
        );
        throw new Error(
          "Application database is missing. Please reinstall or contact support.",
        );
      } else if (fs.existsSync(userDataDbPath)) {
        console.log(
          `[Main Index DB] Database found at ${userDataDbPath}. No copy needed from resources.`,
        );
      }
    }
    ...
    console.error(
      "[Main Index] Error during database initialization process:",
      error,
    );
    dialog.showErrorBox(
      "Database Error",
      `Could not initialize the application database. ${error.message}`,
    );
    try {
      if (app && typeof app.isQuitting === "function" && !app.isQuitting()) {
        ...
        console.error(
          "[Main Index] CRITICAL: Electron 'app.isQuitting' is not a function. Bundling issue suspected. Forcing quit.",
        );
        if (typeof app.quit === "function") app.quit();
        else process.exit(1);
      }
      ...
      console.error(
        "[Main Index] Error trying to quit app after DB init failure:",
        e,
      );
      process.exit(1);
    }
...
async function createWindow() {
  ...
  if (isDev && VITE_DEV_SERVER_URL) {
    console.log(
      `[Main Index] Attempting to load DEV URL: ${VITE_DEV_SERVER_URL}`,
    );
    await mainWindow
      .loadURL(VITE_DEV_SERVER_URL)
      .then(() =>
        console.log(
          `[Main Index] Successfully initiated DEV load for: ${VITE_DEV_SERVER_URL}`,
        ),
      )
      .catch((err) => {
        console.error(
          `[Main Index] FAILED to load DEV URL: ${VITE_DEV_SERVER_URL}`,
          err,
        );
        dialog.showErrorBox(
          "Dev Server Error",
          `Could not connect to Vite dev server at ${VITE_DEV_SERVER_URL}. Ensure it's running.`,
        );
      });
  } else {
    ...
    await mainWindow
      .loadURL(prodUrl)
      .then(() =>
        console.log(`[Main Index] Successfully loaded PROD URL: ${prodUrl}`),
      )
      .catch((err) => {
        console.error(`[Main Index] FAILED to load PROD URL: ${prodUrl}`, err);
        dialog.showErrorBox(
          "Application Error",
          `Could not load the application. URL: ${prodUrl}`,
        );
      });
  }
    ...
    submenu: [
      {
        ...
        click: () => {
          ...
          if (
            ...
            console.warn(
              "[Main Index] Importar carta: No suitable window to trigger dialog for.",
            );
          }
        },
      },
...
if (process.platform === "darwin") {
  const appMenu =
    menuTemplate.find((m) => m.label === app.name) || menuTemplate[0];
...
app
  ...
  .then(async () => {
    ...
    if (protocol && typeof protocol.registerFileProtocol === "function") {
      ...
      protocol.registerFileProtocol("appimg", (request, callback) => {
        try {
          ...
          console.log(
            `[appimg protocol] Request for '${request.url}', resolved to: '${imageFilePath}'`,
          );
          ...
          if (fs.existsSync(imageFilePath)) {
            ...
            console.error(
              `[appimg protocol] File NOT FOUND at resolved path: '${imageFilePath}' for request '${request.url}'`,
            );
            callback({ error: -6 }); // net::ERR_FILE_NOT_FOUND
          }
        } catch (error) {
          console.error(
            `[appimg protocol] Error processing request ${request.url}:`,
            error,
          );
          callback({ error: -6 }); // net::ERR_FILE_NOT_FOUND
        }
      ...
      console.error(
        "[Main Index] CRITICAL: Electron `protocol` module or `registerFileProtocol` is not available!",
      );
      // This is a critical failure if your images rely on this protocol.
      dialog.showErrorBox(
        "Application Error",
        "Failed to set up image loading protocols. The application cannot continue.",
      );
      app.quit();
    ...
    if (!dbInitialized) {
      console.error(
        "[Main Index] Database initialization failed. Quitting app.",
      );
    ...
    app.on("activate", () => {
      if (BrowserWindow.getAllWindows().length === 0) {
        console.log(
          "[Main Index] App activated and no windows open, creating window...",
        );
        createWindow();
      }
    });
  ...
  .catch((initializationError) => {
    ...
    console.error(
      "[Main Index] Unhandled error during app initialization:",
      initializationError,
    );
    dialog.showErrorBox(
      "Critical Application Error",
      `A critical error occurred during startup: ${initializationError.message}. The application will now close.`,
    );
