...
export default async function printTicket(event, data) {
  ...
  const discountRateText = `-${Number(ticket.discount_rate * 100)}%`; // Renamed
  const discountAmountText = `-$${ticket.discount_amount.toFixed(2)}`; // Renamed
  ...
  try {
    function getPrinterInstance() { // Renamed to avoid conflict with escpos.Printer
      ...
      if (usbDevices && usbDevices.length > 0) {
        ...
        device = new escpos.Network(printerIP); // Assign to the outer scope device
        return new escpos.Printer(device);
      }
    }
