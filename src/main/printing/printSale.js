// /src/main/printing/printSale.js
import escpos from 'escpos';
import escposNetwork from 'escpos-network';
import path from 'path'; // Ensure path is imported if not already
...
const _currentFileUrl_printSale = import.meta.url; // Unique suffix
...
export default async function printSale(event, data) {
  ...
  const logoPath = path.resolve(_currentDirname_printSale, '../../static/img/chibi_logo-print.png');
  ...
  const totalSalesAmount = `$${total_sales.toFixed(2)}`; // Renamed variable to avoid conflict with total_sales from destructuring
  const totalSalesPadding = 20 - totalSalesAmount.length - totalLabel.length;

  const discountLabel = 'descuentos:';
  const discountAmountValue = `$${discount_amount.toFixed(2)}`; // Renamed variable
  const discountPadding = 32 - discountAmountValue.length - discountLabel.length;

  const ticketCountLabel = 'transacciones:';
  const ticketCountValue = ticket_count.toString(); // Renamed variable
  const ticketCountPadding = 32 - ticketCountLabel.length - ticketCountValue.length;

  const cancelCountLabel = 'cancelaciones:';
  const cancelCountValue = canceled_count.toString(); // Renamed variable
  const cancelCountPadding = 32 - cancelCountLabel.length - cancelCountValue.length;

  const netLabel = 'TOTAL:';
  const netSalesAmount = `$${net_sales.toFixed(2)}`; // Renamed variable
  const netSalesPadding = 20 - netSalesAmount.length - netLabel.length;

  const avTicketLabel = 'ticket promedio:';
  const avTicketAmount = `$${average_ticket.toFixed(2)}`; // Renamed variable
  ...
  try {
    ...
    printer
        ...
        .size(1, 1)
        .text('-'.repeat(32))
        .text(`${ticketCountLabel}${' '.repeat(ticketCountPadding)}${ticketCountValue}`)
        .feed(1)
        .text(`${cancelCountLabel}${' '.repeat(cancelCountPadding)}${cancelCountValue}`)
        .feed(1)
