...
function decryptImportedMenuForApi(encryptedBase64Menu) {
  try {
    const ciphertext = Buffer.from(encryptedBase64Menu, "base64").toString(
      "latin1",
    );
    ...
    if (!decryptedJson) {
      ...
      throw new Error(
        "Menu decryption failed: output is empty. Check key or data format.",
      );
    }
...
async function openMenuDialog(browserWindow) {
  if (!browserWindow || browserWindow.isDestroyed()) {
    console.warn(
      "[API Module] openMenuDialog: Provided browserWindow is invalid or destroyed.",
    );
    return;
  }
  ...
  try {
    ...
    if (
      ...
      console.warn(
        "[API Module] openMenuDialog: Target window or webContents no longer valid for sending menu-file-opened.",
      );
    }
  } catch (error) {
    console.error(`[API Module] Error reading menu file ${filePath}:`, error);
    dialogInstance.showErrorBox(
      "File Error",
      `Could not read the menu file: ${error.message}`,
    );
  }
}
...
export function initializeApi(
  ...
  ipcMainInstance.handle("get-daily-sale-by-date", async (event, dateParam) => {
    if (!dbModelsInstance.DailySales) {
      console.error(
        "[API get-daily-sale-by-date] DailySales model is not available.",
      );
      return {
        status: 500,
        error: "DailySales model not initialized on server.",
      };
    }
    try {
  ...
  ipcMainInstance.handle(
    ...
    async (event, dateParam) => {
      if (!dbModelsInstance.DailySales) {
        console.error(
          "[API get-daily-sales-by-month] DailySales model is not available.",
        );
        return {
          status: 500,
          error: "DailySales model not initialized on server.",
  ...
  ipcMainInstance.handle("get-daily-sale-report-by-id", async (event, id) => {
    if (
      !dbModelsInstance.DailySales ||
      !dbModelsInstance.DailySalesDetails ||
      !dbModelsInstance.Product
    ) {
    ...
    try {
      ...
      const saleReport = await dbModelsInstance.DailySales.findByPk(id, {
        ...
        include: [
          {
            ...
            include: [
              {
                model: dbModelsInstance.Product,
                as: "productInfo",
                attributes: ["name"],
              },
            ],
        ...
        order: [
          [
            { model: dbModelsInstance.DailySalesDetails, as: "details" },
            { model: dbModelsInstance.Product, as: "productInfo" },
            "name",
            "ASC",
          ],
        ],
  ...
  ipcMainInstance.handle("get-ticket-by-id", async (event, ticketId) => {
    if (
      !dbModelsInstance.Ticket ||
      !dbModelsInstance.TicketDetails ||
      !dbModelsInstance.Product
    ) {
  ...
  ipcMainInstance.handle("cancel-ticket-by-id", async (event, ticketId) => {
    ...
    try {
      ...
      console.error(
        `[API Module] Error canceling ticket ID ${ticketId}:`,
        error,
      );
      return { status: 500, error: error.message };
    }
  ...
  ipcMainInstance.handle(
    ...
    async (event, dateString) => {
      if (
        ...
        !dbModelsInstance.DailySales ||
        !dbModelsInstance.DailySalesDetails ||
        !sequelizeInstanceRef
      ) {
        console.error(
          "[API update-daily-sales-report] Models or Sequelize instance not available.",
        );
      ...
      try {
        ...
        const canceledTicketsData = await dbModelsInstance.Ticket.findOne({
          attributes: [
            [
              sequelizeInstanceRef.Sequelize.fn(
                "COUNT",
                sequelizeInstanceRef.Sequelize.col("id"),
              ),
              "canceled_count",
            ],
          ],
          where: {
            date: { [resolvedOpRef.between]: [startISO, endISO] },
            canceled: true,
          },
          raw: true,
          transaction,
        ...
        const validTicketsData = await dbModelsInstance.Ticket.findOne({
          attributes: [
            ...
            [
              sequelizeInstanceRef.Sequelize.fn(
                "SUM",
                sequelizeInstanceRef.Sequelize.col("cash"),
              ),
              "sum_cash",
            ],
            [
              sequelizeInstanceRef.Sequelize.fn(
                "SUM",
                sequelizeInstanceRef.Sequelize.col("card"),
              ),
              "sum_card",
            ],
          ],
        ...
        const lineItems = await dbModelsInstance.TicketDetails.findAll({
          attributes: [
            ...
            [
              sequelizeInstanceRef.Sequelize.fn(
                "SUM",
                sequelizeInstanceRef.Sequelize.col("quantity"),
              ),
              "sum_quantity",
            ],
            [
              sequelizeInstanceRef.Sequelize.fn(
                "SUM",
                sequelizeInstanceRef.Sequelize.col("extended_price"),
              ),
              "sum_extended_price",
            ],
            [
              sequelizeInstanceRef.Sequelize.fn(
                "SUM",
                sequelizeInstanceRef.Sequelize.col(
                  "ticket_details.discount_amount",
                ),
              ),
              "sum_discount_amount",
            ],
            [
              sequelizeInstanceRef.Sequelize.literal(
                "SUM(ticket_details.price * ticket_details.quantity)",
              ),
              "sum_total_sale_gross",
            ],
          ],
          include: [
            {
              model: dbModelsInstance.Ticket,
              as: "ticketHeader",
              attributes: [],
              where: {
                date: { [resolvedOpRef.between]: [startISO, endISO] },
        ...
        const netSales = lineItems.reduce(
          (sum, item) => sum + parseFloat(item.sum_extended_price || 0),
          0,
        );
        const totalSalesGross = lineItems.reduce(
          (sum, item) => sum + parseFloat(item.sum_total_sale_gross || 0),
          0,
        );
        const discountAmountItems = lineItems.reduce(
          (sum, item) => sum + parseFloat(item.sum_discount_amount || 0),
          0,
        );
        ...
        const canceledCount = parseInt(
          canceledTicketsData?.canceled_count || 0,
          10,
        );

        const reportDateForDB = new Date(dateString + "T00:00:00")
          .toISOString()
          .split("T")[0];
        await dbModelsInstance.DailySales.destroy({
          where: { date: reportDateForDB },
          transaction,
        });
        ...
        const dailySale = await dbModelsInstance.DailySales.create(
          {
            ...
            average_ticket:
              ticketCount > 0 ? Math.round(netSales / ticketCount) : 0,
        ...
        const saleDetails = lineItems.map((item) => ({
          ...
          av_price:
            parseInt(item.sum_quantity, 10) > 0
              ? Math.round(
                  parseFloat(item.sum_extended_price) /
                    parseInt(item.sum_quantity, 10),
                )
              : 0,
          discount_amount: parseFloat(item.sum_discount_amount),
        }));
        if (saleDetails.length > 0) {
          await dbModelsInstance.DailySalesDetails.bulkCreate(saleDetails, {
            transaction,
          });
        }
        ...
        return {
          status: 200,
          dailySale: JSON.parse(JSON.stringify(dailySale)),
        };
      } catch (error) {
  ...
  ipcMainInstance.handle("update-menu-from-import", async (event, menuData) => {
    ...
    try {
      ...
      for (const categoryData of menuData.categories) {
        ...
        const newCategory = await dbModelsInstance.CatProduct.create(
        ...
        if (
          ...
          const productsToCreate = categoryData.products
            .map((productData) => ({
              ...
              status_active:
                productData.status_active !== undefined
                  ? productData.status_active
                  : true,
            }))
            .filter((p) => p.name && p.price >= 0);
          if (productsToCreate.length > 0) {
            await dbModelsInstance.Product.bulkCreate(productsToCreate, {
              transaction,
            });
          }
        }
  ...
  ipcMainInstance.handle(
    "import-update-menu",
    async (event, encryptedBase64Menu) => {
      if (
        !dbModelsInstance.CatProduct ||
        !dbModelsInstance.Product ||
        !sequelizeInstanceRef
      ...
      try {
        ...
        if (
          !decryptedMenu ||
          !Array.isArray(decryptedMenu.cats) ||
          !Array.isArray(decryptedMenu.products)
        ) {
          if (transaction) await transaction.rollback();
          return {
            status: 400,
            error: "Decrypted menu data not in expected format.",
          };
        }
          ...
          if (products.length > 0) {
            await dbModelsInstance.Product.bulkCreate(
              products.map((product) => ({
                ...
                status_active:
                  product.status_active !== undefined
        ...
        return {
          status: 500,
          error: `Failed to import menu. ${error.message.includes("decryption failed") ? "Decryption error." : "Server error."}`,
        };
      }
    },
  );
  ...
  ipcMainInstance.handle("get-ticket-by-date", async (event, dateParam) => {
    ...
    try {
      ...
      console.log(
        `[API get-ticket-by-date] Searching: ${startOfDay.toISOString()} to ${endOfDay.toISOString()}`,
      );
      const tickets = await dbModelsInstance.Ticket.findAll({
        where: {
          date: {
            [resolvedOpRef.gte]: startOfDay,
            [resolvedOpRef.lte]: endOfDay,
          },
        },
        order: [["date", "DESC"]],
      });
  ...
  ipcMainInstance.on("trigger-open-menu-dialog-from-renderer", (event) => {
    ...
    if (!browserWindow) {
      console.warn(
        "[API Module] open-menu-dialog: Could not find BrowserWindow from sender.",
      );
      return;
    }
