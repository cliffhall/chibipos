...
export function initializeSequelize(app, SequelizeConstructor) {
  ...
  if (app.isPackaged) {
    ...
    if (!fs.existsSync(dbDir)) {
      try {
        ...
        console.log(
          `[db/config.js] Created production data directory: ${dbDir}`,
        );
      } catch (error) {
        console.error(
          `[db/config.js] Failed to create production data directory ${dbDir}:`,
          error,
        );
        // Depending on how critical this is, you might want to throw or handle
        throw new Error(
          `Failed to create production data directory: ${error.message}`,
        );
      }
    }
  ...
  async function testConnection() {
    try {
      ...
      console.log(
        "[db/config.js] Database connection has been established successfully.",
      );
    } catch (error) {
