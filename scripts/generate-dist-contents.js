...
const textFileExtensions = new Set([
    '.js', '.mjs', '.cjs', '.html', '.css', '.json', '.yml', '.yaml', '.txt',
    '.md', '.xml', '.svg', '.log', '.ini', '.env', '.sh', '.bat', '.ps1',
    '.ts', '.tsx', '.svelte', '.scss', '.sass', '.less', '.vue', '.jsx', '.map'
]);
...
async function generateReport() {
    ...
    if (!fs.existsSync(distDir)) {
        console.error(`Error: Directory not found at ${distDir}. Please run 'npm run dist:dir' or 'npm run dist:all' first.`);
        process.exit(1);
    }
