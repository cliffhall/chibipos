...
try {
  ...
  const attributePathRegex =
    /(href|src)=(")\/(?!(?:[a-z]+:)?\/\/)((?:_app|favicon\.png|assets|static|images|img)[^"]*)/gi;
  htmlContent = htmlContent.replace(
    attributePathRegex,
    (match, attribute, quote, pathPart) => {
      console.log(
        `[fix-html-paths] Making path relative in attribute: ${match} -> ./${pathPart}`,
      );
      replacementsMade++;
  ...
  const dynamicImportPathRegex =
    /import\((["'])\/(?!(?:[a-z]+:)?\/\/)((?:_app)[^"']*)\1\)/gi;
  htmlContent = htmlContent.replace(
    dynamicImportPathRegex,
    (match, quote, pathPart) => {
      console.log(
        `[fix-html-paths] Making dynamic import path relative: ${match} -> ./${pathPart}`,
      );
      replacementsMade++;
  ...
  const sveltekitBaseObjectRegex =
    /(__sveltekit_\w+\s*=\s*\{[\s\S]*?base\s*:\s*)([^}]+)(\s*[\s\S]*?};)/;
  ...
  if (svelteKitBaseMatch) {
    ...
    if (currentBaseValue === `""` || currentBaseValue === `''`) {
      console.log(
        '[fix-html-paths] SvelteKit base is already empty (""). No change made to SvelteKit base path. Relying on document base URI.',
      );
    } else {
      ...
      htmlContent = htmlContent.replace(
        svelteKitBaseMatch[0],
        newSvelteKitObjectString,
      );
      console.log(
        `[fix-html-paths] SvelteKit base was '${currentBaseValue}', changed to dynamic base: ${dynamicBase}`,
      );
      replacementsMade++;
    }
  } else {
    console.warn(
      "[fix-html-paths] Could not find __sveltekit_... base object in index.html.",
    );
  }
  ...
  if (cspMatch) {
    ...
    if (cspMatch[0] !== modifiedCspTag) {
      ...
      console.log(
        `[fix-html-paths] Modified existing CSP from "${existingCspContent}" to: ${fileProtocolCsp}`,
      );
      replacementsMade++;
    } else {
      console.log(
        `[fix-html-paths] Existing CSP matches target CSP. No change made to CSP.`,
      );
    }
  } else {
    console.warn(
      "[fix-html-paths] Content-Security-Policy meta tag not found. Injecting a default CSP for file:// protocol.",
    );
  ...
  if (replacementsMade > 0) {
    ...
    console.log(
      `[fix-html-paths] Successfully updated HTML in ${absoluteFilePath} (${replacementsMade} replacements).`,
    );
  } else {
    console.log(
      `[fix-html-paths] No replacements made by this script in ${absoluteFilePath}.`,
    );
  }
} catch (error) {
